...
func UTF16BE(str string) []byte {
	...
	err := binary.Write(byteNum, binary.BigEndian, uint16(len(bt)/2))
	if err != nil {
		return nil
	}
...
// ToBinary takes a string and jellyfishes it
...
func DecodeInt64(num []byte) int64 {
	...
	err := binary.Read(bf, binary.BigEndian, &number)
	if err != nil {
		return 0
	}
	return number
...
func DecodeUint64(num []byte) uint64 {
	var number uint64
	bf := bytes.NewReader(num)
	err := binary.Read(bf, binary.LittleEndian, &number)
	if err != nil {
		return 0
	}
	return number
...
func EncodeNum(v interface{}) []byte {
	byteNum := new(bytes.Buffer)
	err := binary.Write(byteNum, binary.LittleEndian, v)
	if err != nil {
		return nil
	}
...
func EncodeNumBE(v interface{}) []byte {
	byteNum := new(bytes.Buffer)
	err := binary.Write(byteNum, binary.BigEndian, v)
	if err != nil {
		return nil
	}
	return byteNum.Bytes()
	...
	if reflect.TypeOf(DataStruct).Kind() == reflect.Slice {
		for i := 0; i < v.Len(); i++ {
			if v.Index(i).Kind() == reflect.Uint8 || v.Index(i).Kind() == reflect.Uint16 || v.Index(i).Kind() == reflect.Uint32 || v.Index(i).Kind() == reflect.Uint64 {
				byteNum := new(bytes.Buffer)
				err := binary.Write(byteNum, binary.LittleEndian, v.Index(i).Interface())
				if err != nil {
					return nil
				}
		...
		for i := 0; i < v.NumField(); i++ {
			if v.Field(i).Kind() == reflect.Uint8 || v.Field(i).Kind() == reflect.Uint16 || v.Field(i).Kind() == reflect.Uint32 || v.Field(i).Kind() == reflect.Uint64 {
				byteNum := new(bytes.Buffer)
				err := binary.Write(byteNum, binary.LittleEndian, v.Field(i).Interface())
				if err != nil {
					return nil
				}
				dumped = append(dumped, byteNum.Bytes()...)
...
// GenerateString creates a random string of length pcount
...
// ReadFileLines loads a text file, trims spaces, skips empty lines.
func ReadFileLines(path string) []string {
	data, err := os.ReadFile(path)
	if err != nil {
		log.Error().Err(err).Msgf("Unable to read file: %s", path)
		return nil
	}
	var lines []string
